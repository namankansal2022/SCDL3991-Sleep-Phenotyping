...
N_EMG_FEATURES = len(EMG_FEATURE_NAMES)  # 15
...
def compute_emg_features_epoch(
...
def extract_emg_features_all_epochs(
    epochs: np.ndarray,
    sampling_rate: float,
) -> np.ndarray:
    """
    ...
    features = np.full((n_epochs, N_EMG_FEATURES), np.nan)

    for i in range(n_epochs):
        features[i] = compute_emg_features_epoch(
            epochs[i],
            sampling_rate,
        )
