...
EEG_FEATURE_NAMES = [
    f"{channel}_{band}"
    for channel in ["EEG1", "EEG2", "EEG3"]
    for band in EEG_BANDS.keys()
]

N_EEG_FEATURES = len(EEG_FEATURE_NAMES)  # 15
...
def compute_eeg_features_epoch(
...
def extract_eeg_features_all_epochs(
    epochs: np.ndarray,
    sampling_rate: float,
) -> np.ndarray:
    """
    ...
    features = np.full((n_epochs, N_EEG_FEATURES), np.nan)

    for i in range(n_epochs):
        features[i] = compute_eeg_features_epoch(
            epochs[i],
            sampling_rate,
        )
